...
---
<!-- stoker-managed: skills:.agents/skills/stoker-create-pr/SKILL.md:f55127da6389cc89 -->
...
summarizes what this PR accomplishes, derived from the branch's
cumulative scope. This is the canonical title rule; step 5 regenerates
the title under the same rule when a PR already exists.
...
5. Regenerate the title from the same cumulative branch scope, applying
   the same <70-character rule as step 4. Do **not** reuse the existing
   PR title — recompute it so it reflects every task that has landed.
